...
- **Sea Chart terrain export under Wine/Proton.** The HeightmapExporter C++ mod's pointer validator (`vp()`) had a 1 TB lower bound that rejected every game-heap address under Wine, where heap allocates at ~4 GB instead of native Windows' >1 TB. Dropped the bound to 64 KB (the Windows allocation granularity floor), which preserves intent on native Windows while accepting Wine's lower heap range. `VirtualQuery`-based page-protection validation in the companion `isR()` check is unchanged. Thanks to @t04glovern for the diagnosis and fix ([#31](https://github.com/HumanGenome/WindrosePlus/pull/31)).
